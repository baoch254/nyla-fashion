...
UNIQUE INDEX `SubCategory.slug_unique`(`slug`),
...
CREATE TABLE `Product` (
    ...
    `imageFront` VARCHAR(191),
    `imageBack` VARCHAR(191),
    `categoryId` INTEGER NOT NULL,
...
CREATE TABLE `Product_Detail` (
    ...
    `price` DOUBLE NOT NULL,
UNIQUE INDEX `Product_Detail.sizeId_colorId_productId_unique`(`sizeId`, `colorId`, `productId`),
...
CREATE TABLE `Order` (
    ...
    `paymentMethod` ENUM('COD', 'PAYPAL') NOT NULL,
    `tax` DOUBLE NOT NULL,
    `shippingFee` DOUBLE NOT NULL,
    `shippingAddressId` INTEGER NOT NULL,
    `totalPayment` DOUBLE NOT NULL,
    `note` TEXT NOT NULL,
UNIQUE INDEX `Order_shippingAddressId_unique`(`shippingAddressId`),
...
-- AddForeignKey
ALTER TABLE `Product` ADD FOREIGN KEY (`categoryId`) REFERENCES `SubCategory`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Order` ADD FOREIGN KEY (`shippingAddressId`) REFERENCES `Order_Shipping`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
